...
const ImageCard = ({ imageData, imageIndex, onImageClicked }) => {
  const { urls, alt_description, user, links, color, blur_hash } = imageData
    ...
    <div className="masonry-brick" style={{ display: "none" }}>
      ...
      >
        <img
          ...
          alt={alt_description}
        />
        <div className="image-card--options" style={{ height: 60 }}>
          <div className="user-info">
            ...
            <h3 class="has-text-white">{user.name}</h3>
          </div>
          <div className="links">
            <a
              className="link"
        ...
        /> */}
        <BlurhashCanvas className="image-card-fg" hash={blur_hash} punch={1} />
      </div>
    </div>
  )
}
